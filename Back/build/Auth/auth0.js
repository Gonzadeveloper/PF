...
app.post("/user", checkJwt, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    let { name, email, password, typeuser, address, country, city, state, postalcode, } = req.body.user;
    ...
    try {
        ...
        if (result.rows.length === 0) {
            ...
            yield client.query("INSERT INTO users (name, email, password, typeuser, address, country, city, state, postalcode) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)", [
                name,
                email,
                password,
                typeuser,
            ...
            yield client.query("UPDATE users SET name = $1, password = $2, typeuser = $3, address = $4, country = $5, city = $6, state = $7, postalcode = $8 WHERE email = $9", [
                name,
                password,
                typeuser,
                address,
